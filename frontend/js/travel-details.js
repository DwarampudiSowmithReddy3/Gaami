...
class TravelDetails {
    ...
    async fetchPlacesToVisit() {
        try {
            const response = await fetch(`http://localhost:5001/api/places?destination=${encodeURIComponent(this.destination)}`);
    ...
    async fetchHotels() {
        try {
            const response = await fetch(`http://localhost:5001/api/hotels?destination=${encodeURIComponent(this.destination)}`);
    ...
    async fetchFoodRecommendations() {
        try {
            const response = await fetch(`http://localhost:5001/api/food?destination=${encodeURIComponent(this.destination)}`);
